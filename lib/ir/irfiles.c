...
#ifndef lint
static char *PRCSid = "$Id: irfiles.c 1.6.1.13.1.3 Mon, 05 May 1997 11:54:27 +0200 pfeifer $";
#endif /* not lint */
 ...
 *
 * $Log: irfiles.c,v $
 * Revision 2.3  1997/03/07 16:31:34  pfeifer
 * Minor fixes for next. Stuff for compilation without Perl
 *
 * Revision 2.2  1997/02/04 17:12:45  pfeifer
 * Switched to CVS
 *
 * Revision 2.0.1.10  1996/12/27 16:17:50  pfeifer
 * patch69: Support for any_stemmer. Removed handle_numeric().
 *
...
/* close a database and all its files. Do not dispose of the structure. */
{
  ...
  { /* destruct the perl interpreter in grundform.c */
    extern void exit_perl();
    exit_perl();
  }
}
...
#else
#define NAN(s) ((!(*s) || !strncmp(s,DICTIONARY_TOTAL_SIZE_WORD, \
                                 strlen(DICTIONARY_TOTAL_SIZE_WORD))))
#define ATOF(s) ((*s=='-')?-atof((s)+1):atof(s))
     ...
     database* db;
{
  ...
  for(i=0; i < number_of_elements; i++) {
    ...
    for (j=0;j<block_length;j++) {
      ...
      if (NAN(dictionary_word)) break;

      if(isnumchar(dictionary_word[0])) {
        double dictionary_number = ATOF(dictionary_word);
	...
	*position = i - 1;
	...
	if (findpart_init)
	  ...
	  if (!strcmp (word, DICTIONARY_TOTAL_SIZE_WORD) &&
	      ...
	      (
#ifdef STEM_WORDS
		(IS_STEMMED(dictionary_word[dctwrd_len - 1])) ||
#endif /* STEM_WORDS */
		(dictionary_word[dctwrd_len - 1] == NUMERIC_TAG))) {
	    if (IS_STEMMED(dictionary_word[dctwrd_len - 1]))
	      stemming = dictionary_word[dctwrd_len - 1];
	    else
	      gnumeric = true;
	    dictionary_word[dctwrd_len - 1] = '\0';
	  }
...
#ifdef STEM_WORDS
  if(stemming && !db->fields[field_id].stemming) {
     ...
     /* Puts a word into the dictionary file. */
{
  ...
  if(strcmp(word, DICTIONARY_TOTAL_SIZE_WORD) == 0) {
    if(db->stemming) 
      padded_word[strlen(padded_word)] = 
        IS_STEMMED(db->stemming)?db->stemming:ENGLISH_STEMMING_TAG;
  }
#endif /* STEM_WORDS */
     ...
     /* Puts a word into the dictionary file. */
{
  ...
  if(strcmp(word, DICTIONARY_TOTAL_SIZE_WORD) == 0)
	...
	padded_word[strlen(padded_word)] = 
          IS_STEMMED(db->fields[field_id].stemming)?
          db->fields[field_id].stemming:ENGLISH_STEMMING_TAG;
      else
#endif /* STEM_WORDS */
	if(db->fields[field_id].numeric)
