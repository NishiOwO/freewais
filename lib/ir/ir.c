/*                               -*- Mode: C -*- 
 * $Basename: ir.c $
 * $Revision: 1.7.1.10.1.3 $
 * Author          : Jonny Goldman
 * Created On      : 92/02/12  13:20:49
 * Last Modified By: Ulrich Pfeifer
 * Last Modified On: Mon May  5 11:59:07 1997
 * Language        : C
 * Update Count    : 2
...
#ifndef lint
static char *PRCSid = "$Id: ir.c 1.7.1.10.1.3 Mon, 05 May 1997 18:03:26 +0200 pfeifer $";
#endif
     ...
     boolean results;
{
  ...
  hit catalog;
  char local_id[MAX_FILENAME_LEN + 60];
  WAISDocumentHeader *temp = NULL; /*memory leak fix 08/13/96 JF */
  ...
  if(probe_file(catalog.filename))
	...
	length = file_length(stream);
      ...
      temp=makeWAISDocumentHeader(anyFromDocID(theDocID),
				    UNUSED,
...
{ 
  ...
  *sendBuf = writeSearchResponseAPDU(response,*sendBuf,sendBufLen);
  ...
  /* francois - locks */
  closeDatabase(db);
  disposeDatabase(db); /* Memory leak fix JF 08/14/96 */
}
