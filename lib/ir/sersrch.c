/*                               -*- Mode: C -*- 
 * $Basename: sersrch.c $
 * $Revision: 1.5.1.16.1.4 $
 * Author          : Jonathan Goldman
 * Created On      : Fri Jun 23 14:05:18 1995
 * Last Modified By: Ulrich Pfeifer
 * Last Modified On: Mon May  5 11:10:25 1997
 * Language        : C
 * Update Count    : 51
...
#ifdef PROXIMITY
/* This function merges adjacent elements in the         */
/* provided document array if they have the same doc_id. */
long
merge_docs (doc_array, count)
     doc_descr_struct *doc_array;
     long count;
{
  long cur;   /* Current element being examined.              */
  long last;  /* Last element that has already been examined. */
  for (last = cur = 0; cur < count; last++) {
    long end = cur;
    long total_words = 0;
    while (end < count && doc_array [cur].doc_id == doc_array [end].doc_id) {
      total_words += doc_array [end].numwords;
      end++;
    }
    if (--end != cur) {
      long *temp_words;
      ...
      long i;
      long doc_id;
      /* Multiple occurrences of the same document result when the */
      /* search word matches multiple words in the dictionary, so  */
      /* adjust the score as if those words were OR-ed together.   */
      doc_array [last].score = doc_array [cur].score;
      for (i = cur + 1; i <= end; i++)
          doc_array [last].score += doc_array [i].score;
      /* Allocate space for a new word position list and */
      /* create that list by merging the word positions  */
      /* lists while preserving their increasing order.  */
      temp_words = s_malloc (total_words * sizeof (long));
      if (temp_words == NULL)
      return -1;
      /* Hack Alert: the following code reuses the doc_id */
      /* field as a current position marker to avoid      */
      /* having to allocate an array of such markers.     */
      doc_id = doc_array [cur].doc_id;
      for (i = cur; i <= end; i++)
        doc_array [i].doc_id = 0;
      temp_pos = 0;
      for (;;) {
      long smallest = -1;
      /* Find the smallest word position among the current candidates. */
      for (i = cur; i <= end; i++)
        if (doc_array [i].doc_id < doc_array [i].numwords)
          if (smallest == -1)
            smallest = i;
          else if (doc_array [i].word_position [doc_array [i].doc_id] <
           doc_array [smallest].word_position [doc_array [smallest].doc_id])
            smallest = i;
      /* Copy the smallest word position to the merged word position list */
      /* if there was one and it's not a duplicate of the previous one. */
      if (smallest == -1)
        break;
      else {
        if (temp_words [temp_pos] ==
          doc_array [smallest].word_position [doc_array [smallest].doc_id]) {
          doc_array [smallest].doc_id++; /* Discard the duplicate. */
        }
        else {
          temp_words [temp_pos++] = doc_array [smallest].word_position
                                      [doc_array [smallest].doc_id++];
        }
      }
      }
      /* Free the now useless word position lists. */
      for (i = cur; i <= end; i++)
      s_free (doc_array [i].word_position);
      doc_array [last].word_position = temp_words;
      doc_array [last].numwords = temp_pos;
      doc_array [last].doc_id = doc_id;
      cur = end + 1;
    }
    else {
      if (cur != last)
        memcpy (&doc_array [last], &doc_array [cur], sizeof doc_array [cur]);
      cur++;
    }
  }
  return last;
...
#ifdef STEM_WORDS
    if (field_id > -1) {
	any_stemmer (db->fields[field_id].stemming, word);
    } else {
	any_stemmer (db->stemming, word);
    }
#endif /* STEM_WORDS */
  }
#ifdef LITERAL
...
#ifdef STEM_WORDS
	  if (field_id > -1) {
	      any_stemmer (db->fields[field_id].stemming, word1);
	  } else {
	      any_stemmer (db->stemming, word1);
	  }
#endif /* STEM_WORDS */
...
#ifdef PROXIMITY
  search_result_array [operand_id].number_of_hits =
      merge_docs (search_result_array [operand_id].doc_ids_array,
                  search_result_array [operand_id].number_of_hits);
#endif
  if (!save_operand_id (operand_id, search_result_array))
    return (-1);
