/*                               -*- Mode: C -*- 
 * $Basename: irtfiles.c $
 * $Revision: 1.7.1.8.1.4 $
 * Author          : Jonny Goldman
 * Created On      : 6/90
 * Last Modified By: Ulrich Pfeifer
 * Last Modified On: Mon May  5 11:20:11 1997
 * Language        : C
 * Update Count    : 28
...
#define char_downcase(A) (A)
	...
	if (position_in_word >= minwordlen) {	/* is it reasonable ? */
	  ...
	  if (db->number_of_fields > 0) {
#ifdef STEM_WORDS
	      any_stemmer (index_stemming, word);
	  } else {
...
static int nodecompare (const void  *i, const void *j);
...
nodecompare (i,j)
     ...
     const void  *j;
{
const unsigned long *k = i;
const unsigned long *l = j;
  if (k[0] < l[0])
    return (-1);
  else if (k[0] > l[0])
    return (1);
