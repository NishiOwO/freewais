            ...
            if (db->index_fields[field_id]->numeric) {  
                ...
                if (db->index_fields[field_id]->is_date) { 
                    ...
                    if (db->index_fields[field_id]->numeric_len > 0) {
                        ...
                        sscanf(index_line, "%lf", &num);
                        file_position_before_line += strlen(index_line);
                        sprintf(index_line, "%lf", num);
                        file_position_before_line -= strlen(index_line);
                    }
                }
            }
     ...
     char           *type;
{
  ...
  while (indextypes[i]) {
	...
	index_stemming = '\0';
      ...
      /* used in map_over_words of irtfiles.c */
	index_stemming = db->fields[field_id].stemming;	
      else
	index_stemming = '\0';
#endif /* STEM_WORDS */
      number_of_words = map_over_words (line, document_id,
