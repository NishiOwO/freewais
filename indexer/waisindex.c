/*                               -*- Mode: C -*- 
 * $Basename: waisindex.c $
 * $Revision: 1.7.1.9.1.4 $
 * Author          : Brewster Kahle, Jonny Goldman
 * Created On      : 6/90
 * Last Modified By: Ulrich Pfeifer
 * Last Modified On: Mon May  5 10:48:10 1997
 * Language        : C
 * Update Count    : 27
...
#ifndef lint
static char *PRCSid = "$Id: waisindex.c 1.7.1.9.1.4 Mon, 05 May 1997 11:54:27 +0200 pfeifer $";
#endif
...
/* $Format: "#define INDEXER_DATE \"$Date$\""$ */
#define INDEXER_DATE "Mon, 05 May 1997 11:54:27 +0200"
...
{				/* no args */
  ...
  fprintf (stderr, "          [-grundform] /* german grundform for global database */\n");
#endif
  ...
  fprintf (stderr, "          [-nosourcelist] /* Do not include the list of source files in the .src file; waisq barfs if :description is > 64KB! */\n");
     ...
     char           *argv[];
{
  ...
  while ((next_argument != NULL) && '-' == next_argument[0]) {
	...
	exit (0);
      ...
      stemming = ENGLISH_STEMMING_TAG;
    }
    else if (0 == strcmp ("-grundform", next_argument)) {
      stemming = GERMAN_GRUNDFORM_TAG;
    }
#endif
	...
	word_positions = true;
      ...
      make_catalog = false;
    } else if (0 == strcmp ("-nosourcelist", next_argument)) {
      nosourcelist = true;
...
*/
	...
	panic ("Expected a file type");
      ...
      dataops.type = strdup(next_argument);
  ...
  }
  {
    ...
    if (!probe_file (source_filename (filename, db))) {
      ...
      write_src_structure (source_filename (filename, db),
			   ...
			   typename,
			   &argv_copy[start_of_filenames], /*filenames*/
			   nosourcelist? 0 : (argc_copy - start_of_filenames), /*count*/
			   export_database,
			   210L, db);
    } else {
      char           *oldkeys[50];
